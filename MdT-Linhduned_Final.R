...
#               http://mdgbeck.netlify.com/post/tidytext-analysis-of-seinfeld/              #
#                                                                                           #
...
# Atención:
# Hay que reiniciar sesión, de otro modo daríoa un error. Lo mejor es:
# Session > RStart R
# y después hacer clic en la escoba de Environment y PLots. Te ahorrarás un susto.
...
# Carga el texto
MdT <- read_delim("MdT.txt", delim = "\t")
MdT <- MdT %>%
  mutate(temporada = as.numeric(temporada)) %>%
  mutate(episodio = as.numeric(episodio)) %>%
  mutate(titulo = as.factor(titulo))

# Borra líneas en blanco y añade una nueva variable personaje2
# en la que aquellos que no son los principales, se reunen bajo
# ZESTO (es un mal truco para que sea la última variable)
MdT <- MdT %>% 
  filter(texto != "" & !is.na(texto)) %>% 
  mutate(personaje2 = ifelse(
    personaje %in% c("PACINO", "JULIÁN", "AMELIA", "ALONSO", "SALVADOR", "ERNESTO"),
    personaje, "ZESTO"))
...
frecuencia <- guion_tidy %>% 
  ...
  gather(personaje, proporcion, ALONSO:SALVADOR)
...
# En los seis gráficos, las palabras que se sitúan a lo largo de la línea de puntos
...
# que usan más los del ZESTO (excluidos los otros cinco personajes principales).


# Ahora vamos hacer un análisis de correlación para ver la similitud de la frecuencia
# de las palabras que usa los seis personajes principales comparados con el resto (ZESTO)
# de los personajes
...
frecuencia %>% 
  ...
  labs(x = "coeficiente de correlación",
       ...
       subtitle = "Patrulla en comparación al resto de los personajes") +
  theme(legend.position = "none",
        axis.title.y = element_blank())

# Los seis protagonista tienen una correlación bastante alta entre ellos. Pero Amelia se
# separa un tanto de los personajes masculinos. ¿Quiere decir que los guionistas supieron
# hacer que Amelia hablara como una mujer?
...
episodios_top <- palabras_episodio %>% 
...
bigramas_separados <- bigramas_MdT %>% 
  separate(bigrama, c("palabra1", "palabra2"), sep = " ")

bigramas_filtrados <- bigramas_separados %>% 
  filter(!palabra1 %in% vacias_esp$palabra) %>% 
  filter(!palabra2 %in% vacias_esp$palabra) %>% 
